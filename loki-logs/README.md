...
how to use it. Part of the [`tiger-seo/claude-skills`](https://github.com/tiger-seo/claude-skills)
monorepo.
...
```
git clone https://github.com/tiger-seo/claude-skills.git
```

Windows:
```
cmd /c mklink /J "%USERPROFILE%\.claude\skills\loki-logs" "D:\path\to\claude-skills\loki-logs"
```

macOS/Linux:
```
ln -s /path/to/claude-skills/loki-logs ~/.claude/skills/loki-logs
```

Note the symlink points at the `loki-logs/` subfolder, not the repo root. Invoked simply as
`/loki-logs`.

**Option 3 — project skill:** clone this repo elsewhere and symlink (or copy) its `loki-logs/`
subfolder into `<your-repo>/.claude/skills/loki-logs`.
...
Commands below assume your working directory is this `loki-logs/` folder.
